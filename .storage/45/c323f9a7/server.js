...
const { setupMetrics, metrics } = require('./monitoring');
...
connectDB().then(() => {
  ...
  // Update active users metric (example: could be replaced with actual user session tracking)
  setInterval(() => {
    // This would typically be based on active sessions or connected clients
    const randomActiveUsers = Math.floor(Math.random() * 100);
    metrics.activeUsers.set(randomActiveUsers);
  }, 5000);
